...
- [[file:newbiemacs_cheat_sheet_windows.pdf][Cheat sheet for VIM style]]
...
- To enable Vim, in the Newbiemacs screen type ~e~ and choose "vim".
- Once you enable Vim, any shortcut prefix ~M-o~ can be replaced by ~SPC~ and any shortcut prefix ~M-RET~ can be replaced by ~,~ in normal mode.

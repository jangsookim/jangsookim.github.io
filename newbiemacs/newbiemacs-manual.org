...
4. Type ~SPC f S~. Then it will ask you to enter a symlink name. Follow the instructions.
...
- Using Newbiemacs you can add an image into your tex file as follows.
  ...
  3. Press ~, i f~ and follow the instructions.
...
- In Newbiemacs you can do the variable change as follows.
  ...
  3. Press ~, t v~ and follow the instructions.
...
- You can convert this latex code to HWP math code as follows.
  ...
  2. Press ~, t h~ and a converted HWP math code will be stored in the
...
2. In a latex file, type ~, B n~ and follow the instructions.
...
3. In a latex file, type ~, B n~ and follow the instructions.
...
*** Toggle between main bib and local bib
- Type ~, B t~
*** Add one bib item to the existing local-ref.bib file
1. Move the cursor at the bib key inside \cite{..}.
2. Type ~, B l~ to add the bib item from the main bib file to the local bib file.
* Org-mode
